...
const AccountProfile: FC<AccountProfileProps> = ({ user, btnTitle }) => {
  ...
  async function onSubmit(values: UserSchemaType) {
    ...
    const { isBase64Image } = await import("@/lib/utils");
  ...
  async function handleImage(
    ...
    const { isImageValid } = await import("@/lib/utils");
    const validityResponse = isImageValid(file);
    if (!validityResponse.success) {
      return alert(validityResponse.message);
    }
    ...
    reader.onload = (e) => {
      fieldChange(e.target?.result?.toString() || "");
    };
  }
